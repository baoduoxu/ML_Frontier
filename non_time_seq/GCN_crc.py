...
### Construct the graph
def construct_knn_graph(X, y, val_idx, k=5, metric='unsupervised', ratio=0.3, drop_class=1, density=0.1, drop_edges=230):
    '''
    ...
    for i in range(len(X)):
        for j in range(i+1,len(X)):
            # 随机断开验证集和测试集内的点与连边
            if adj_mat[i][j]==1 and y[i]==y[j]: # 对于训练集中的数据，只有两个是同一类才连边
